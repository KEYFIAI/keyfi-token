...
/**
 * MultisigTimelock contract for KeyFi admin key. Based on DDEX MultiSigWalletWithTimelock by 
 * The Hydro Protocol Foundation (Copyright 2019).
 * 
 * Implements a multisig wallet with timelock functionality, configurable by their owners.
 */
contract MultisigTimelock {
    ...
    struct Transaction {
        address destination;
        uint256 value;
        bytes data;         // e.g. keccak256(abi.encodePacked("setMarketBorrowUsability(uint16,bool)"))
        bool executed;
    }
